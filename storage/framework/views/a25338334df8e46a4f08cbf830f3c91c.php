<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title><?php echo $__env->yieldContent('title', 'Bridge India Capital — Strategic Capital Infrastructure'); ?></title>
    <meta name="description" content="<?php echo $__env->yieldContent('description', 'Bridge India Capital — strategic investment and M&A platform for startups, MSMEs and investors.'); ?>">
    <meta name="robots" content="<?php echo $__env->yieldContent('robots', 'index,follow'); ?>">
    <link rel="canonical" href="<?php echo $__env->yieldContent('canonical', url()->current()); ?>">
    <meta property="og:type" content="<?php echo $__env->yieldContent('og_type', 'website'); ?>">
    <meta property="og:title" content="<?php echo $__env->yieldContent('title', 'Bridge India Capital — Strategic Capital Infrastructure'); ?>">
    <meta property="og:description" content="<?php echo $__env->yieldContent('description', 'Bridge India Capital — strategic investment and M&A platform for startups, MSMEs and investors.'); ?>">
    <meta property="og:url" content="<?php echo $__env->yieldContent('canonical', url()->current()); ?>">
    <meta property="og:image" content="<?php echo $__env->yieldContent('og_image', asset('favicon.ico')); ?>">
    <link rel="icon" type="image/svg+xml" href="<?php echo e(asset('favicon.svg')); ?>">
    <link rel="icon" type="image/x-icon" href="<?php echo e(asset('favicon.ico')); ?>">
    <link rel="apple-touch-icon" href="<?php echo e(asset('apple-touch-icon.png')); ?>">
    <?php echo app('Illuminate\Foundation\Vite')(['resources/css/app.css', 'resources/js/app.js']); ?>
    <?php echo $__env->yieldPushContent('head'); ?>
</head>
<body>
    <?php echo $__env->yieldContent('content'); ?>

    <?php if(session('success')): ?>
        <div id="bic-server-flash" data-type="success" data-title="Action Completed Successfully" data-message="<?php echo e(session('success')); ?>" hidden></div>
    <?php elseif(session('error')): ?>
        <div id="bic-server-flash" data-type="error" data-title="Action Unsuccessful" data-message="<?php echo e(session('error')); ?>" hidden></div>
    <?php endif; ?>
</body>
</html><?php /**PATH C:\Users\DESIGN\Downloads\BIC_Laravel_Production_Source\resources\views/layouts/app.blade.php ENDPATH**/ ?>