...
<script>
            ...
            if (data.success && data.action) {
                ...
                const itemHtml = `
                    <div class="lead-action-card" id="action-item-${data.action.id}">
                        ...
                        <div class="action-card-controls">
                            <button type="button" class="action-card-edit-btn" onclick="editActionInline(${data.action.id}, ${leadId}, '${escapeHtml(data.action.action).replace(/'/g, "\\'")}', '${escapeHtml(data.action.timeline || '').replace(/'/g, "\\'")}', '${escapeHtml(data.action.state).replace(/'/g, "\\'")}')" title="Edit this action">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                            </button>
    ...
    function editActionInline(actionId, leadId, currentAction, currentTimeline, currentState) {
        ...
        cardMain.innerHTML = `
            <form onsubmit="submitEditAction(event, ${actionId}, ${leadId})" class="inline-edit-form" style="display: flex; flex-direction: column; gap: 6px; width: 100%;">
                ...
                <select name="state" class="quick-input-state" style="padding: 6px; font-size: 13px; border: 1px solid var(--line); border-radius: 4px; background: white;">
                    <option value="In Progress" ${!isComplete ? 'selected' : ''}>In Progress</option>
                    <option value="Complete" ${isComplete ? 'selected' : ''}>Complete</option>
                </select>
    ...
    async function submitEditAction(event, actionId, leadId) {
        ...
        const stateVal = form.querySelector('[name="state"]').value;
        ...
        try {
            ...
            const response = await fetch(`/admin/leads/actions/${actionId}`, {
                ...
                body: JSON.stringify({ action: actionVal, timeline: timelineVal, state: stateVal })
            });
            const data = await response.json();
            if (data.success) {
